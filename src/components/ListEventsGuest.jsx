        ...
        <div className="event-card-holder">
            {events.map(event => (
                <div id={"card" + event.id} key={event.id} className="event-card secondary">
                    ...
                    <div id={"secondary-info" + event.id} className={classNames("secondary-info" + event.id, "hidden", "event-card-section", "primary")}>
                    ...
                    <button onClick={() => handleExpand(event.id)} href={"#arrow" + event.id} className='button expand-button primary'><span id={"arrow" + event.id} style={{padding: "10px", borderRadius: "50%"}} className="arrow material-symbols-rounded primary">expand_more</span></button>
                </div>
            ))}
        </div>
    </>
...
export default ListEventsGuest